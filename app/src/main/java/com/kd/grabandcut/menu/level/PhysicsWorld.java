...
public class PhysicsWorld extends SurfaceView {
  ...
  public PhysicsWorld(Context context, int W, int H, Level level) {
    ...
    world.setContactFilter(new ContactFilter() {
      ...
      @Override public boolean shouldCollide(Shape shape1, Shape shape2) {
        return false;
      }
    });
    ...
    restart = new Rect();
    restart.set((int) (200 * World_W / 320), (int) (5 * World_H / 480), (int) (240 * World_W / 320),
        (int) (35 * World_H / 480));
    mainmenu = new Rect();
    mainmenu.set((int) (260 * World_W / 320), (int) (5 * World_H / 480), (int) (300 * World_W / 320),
        (int) (35 * World_H / 480));
    main = new Rect();
    main.set((int) (140 * World_W / 320), (int) (5 * World_H / 480), (int) (180 * World_W / 320),
        (int) (35 * World_H / 480));
  ...
  public void addstar(int x, int y) {
    ...
    star[Starnumber].set(x - (int) (22 * World_W / 320), y - (int) (20 * World_H / 480), x + (int) (20 * World_W / 320),
        y + (int) (22 * World_H / 480));
  ...
  public void addtimestar(int x, int y, int timemillisecond) {
    ...
    timestar[timestarnumber].set(x - (int) (10 * World_W / 320), y - (int) (10 * World_H / 480),
        x + (int) (10 * World_W / 320), y + (int) (10 * World_H / 480));
  ...
  public void addcutter(int x, int y, int height, int width, int movex, int movey) {
    cutter[cutternumber] = new Rect();
    cutter[cutternumber].set(x, y, x + width, y + height);
  ...
  public void addbubble(int x, int y) {
    ...
    bubble[bubblenumber].set(x - (int) (25 * World_W / 320), y - (int) (25 * World_H / 480),
        x + (int) (25 * World_W / 320), y + (int) (25 * World_H / 480));
  ...
  public void adddoormover(int x, int y, int moveinx, int moviny) {
    ...
    doormover[doormovernumber].set(x - (int) (10 * World_W / 320), y - (int) (10 * World_H / 480),
        x + (int) (10 * World_W / 320), y + (int) (10 * World_H / 480));
  ...
  public void adddoor(int x, int y) {
    ...
    door.set(x - (int) (30 * World_W / 320), y - (int) (35 * World_H / 480), x + (int) (30 * World_W / 320),
  ...
  public void drawball(Canvas canvas) {
    ...
    if (Finalbody != null) {
      Rect r = new Rect();
      r.top = (int) Finalbody.getPosition().y - (int) (25 * World_H / 480);
      r.bottom = (int) Finalbody.getPosition().y + (int) (25 * World_H / 480);
      r.left = (int) Finalbody.getPosition().x - (int) (25 * World_W / 320);
      r.right = (int) Finalbody.getPosition().x + (int) (25 * World_W / 320);

      if (vootdestroyed == false) {
        if (ballbubblestate == false) {

          canvas.drawBitmap(character[charracternumber], null, new Rect(r), null);
          if (starflag > 8) {
            charracternumber++;
            starflag = 1;
          }
  ...
  public void drawrope(Canvas canvas, int index) {
    ...
    if (flag) {
      ...
      for (int i = 1; i < 6; i++) {
        ...
        paint.setColor(Color.argb(100, redcolor, greencolor, bluecolor));
        canvas.drawLine(startx - 2, starty, body.getPosition().x - 2, body.getPosition().y, paint);
        paint.setColor(Color.argb(200, redcolor, greencolor, bluecolor));
        canvas.drawLine(startx - 1, starty, body.getPosition().x - 1, body.getPosition().y, paint);
        paint.setColor(Color.argb(255, redcolor, greencolor, bluecolor));
        canvas.drawLine(startx, starty, body.getPosition().x, body.getPosition().y, paint);
        canvas.drawLine(startx, starty, body.getPosition().x, body.getPosition().y, paint);
        paint.setColor(Color.argb(200, redcolor, greencolor, bluecolor));
        canvas.drawLine(startx + 1, starty, body.getPosition().x + 1, body.getPosition().y, paint);
        paint.setColor(Color.argb(100, redcolor, greencolor, bluecolor));
        ...
        boj = body;
        paint.setColor(Color.RED);
        canvas.drawCircle(body.getPosition().x, body.getPosition().y, 4, paint);
      }
      ropeattached++;
    }
    if (boj != null) {
      ...
      if (timestar[i].right != 0) {
        if (time[i] < 0) {
          ...
          if (timestar[i].right < (int) (320 * World_W / 320)) {
            ...
            canvas.drawBitmap(starpic[starpicnum], null, timestar[i], null);
          }
        } else {
          canvas.drawBitmap(starpic[starpicnum], null, timestar[i], null);
          time[i]--;
  ...
  public void drawcutter(Canvas canvas) {
    ...
    for (int i = 0; i < cutternumber; i++) {
      ...
      if (cutter[i].top < 0 || cutter[i].bottom > (int) (480 * World_H / 480)) {
        cuttermoveratey[i] = cuttermoveratey[i] * (-1);
      }
      if (cutter[i].left < 0 || cutter[i].right > (int) (320 * World_W / 320)) {
      ...
      cutter[i].left = cutter[i].left - (int) (cuttermoveratex[i] * World_W / 320);
      cutter[i].right = cutter[i].right - (int) (cuttermoveratex[i] * World_W / 320);
      // canvas.drawRect(cutter[i], paint);
      canvas.drawBitmap(cutterpic[cutterpicnumber], null, cutter[i], null);
    }
    cutterpicnumber++;
  ...
  public void drawropecutter(Canvas canvas) {
    ...
    for (int i = 0; i < ropecutternumber; i++) {
      ...
      canvas.drawBitmap(ropecutterpic[ropecutterpicnum], null, ropecutter[i], null);
    }
    ropecutterpicnum++;
  ...
  @Override public boolean onTouchEvent(MotionEvent event) {
    ...
    finished = 1;
    AABB aabb = new AABB();
    aabb.lowerBound.set((float) (x - (5 * World_W / 320)), (float) (y - (5 * World_H / 480)));
    aabb.upperBound.set((float) (x + (5 * World_W / 320)), (float) (y + (5 * World_H / 480)));
  ...
  public void collision() {
    ...
    Rect r = new Rect();
    r.set(x - (int) (20 * World_W / 320), y - (int) (20 * World_H / 480), x + (int) (20 * World_W / 320),
        y + (int) (20 * World_H / 480));
    for (int i = 0; i < Starnumber; i++) {
      if (star[i].right != 0) {
    ...
    doornear.set(door.left - (int) (20 * World_W / 320), door.top - (int) (20 * World_H / 480),
        door.right + (int) (20 * World_W / 320), door.bottom + (int) (20 * World_H / 480));
    ...
    for (int i = 0; i < ropecutternumber; i++) {
      ...
      // canvas.drawBitmap(buuble, null, bubble[i], null);
      AABB aabb = new AABB();
      aabb.lowerBound.set((float) (ropecutter[i].right), (float) (ropecutter[i].bottom));
      aabb.upperBound.set((float) (ropecutter[i].left), (float) (ropecutter[i].top));
      Shape[] s = new Shape[10];
      s = world.query(aabb, 10);
  ...
  public void handle_ondraw(Canvas canvas) {
    if (canvas == null) {
      return;
    }
    ...
    canvas.drawBitmap(Backgorund[0], null, new Rect(xx, yx, xx + 90, yx + 90), null);
    ...
    for (int i = 0; i < Starnumber; i++) {
      if (i < starcollection) {
        canvas.drawBitmap(starpic[starpicnum], null, new Rect(5 + i * 20, 5, 25 + i * 20, 25), null);
      } else {
        canvas.drawBitmap(torch[torchnumber], null, new Rect(5 + i * 20, 5, 25 + i * 20, 25), null);
      }
    }
    drawtouch(canvas);
  ...
  public void drawbackground(Canvas canvas) {
    ...
    for (int i = 0; i < 40; i++) {
      ...
      canvas.drawCircle(xs[i], ys[i], rs[i] + (int) (Math.random() * 2), paint);
    }
    paint.setColor(Color.WHITE);
    starflag++;
    if (starflag == 12) {
  ...
  public void Update(Canvas canvas) {
    ...
    collision();
    handle_ondraw(canvas);
    // Log.d("LevelScreen"+Finalbody.getPosition().y,
    // "Reset Level"+Finalbody.getPosition().x);
    if (Finalbody != null) {
      if (ropebool == false && Finalbody.getPosition().y > (int) ((World_H))) {
        finished();
      }
      if (Finalbody.getPosition().y < (int) (-30)) {
        finished();
      }
      if (ropebool == false && Finalbody.getPosition().x > (int) (World_W + 30)) {
        finished();
      }
      if (ropebool == false && Finalbody.getPosition().x < (int) ((-30))) {
        finished();
      }
    }
    //		}
  ...
  private void reset() {
    // /db write
    gs = new GameHistory(TestGame.currentLevelIndex + 1, 1, starcollection, score);
